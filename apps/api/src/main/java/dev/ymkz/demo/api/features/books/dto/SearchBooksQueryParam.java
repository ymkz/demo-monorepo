...
import dev.ymkz.demo.api.features.books.BookOrder;
import dev.ymkz.demo.api.features.books.BookSearchQuery;
...
public record SearchBooksQueryParam(
        @Parameter(description = "ISBN-13:完全一致", example = "9784873115658")
        String isbn,
        ...
        Instant publishedAtStart,

        @Parameter(description = "出版日時:ISO8601:終了", example = "2025-01-23T01:23:45.000Z")
        Instant publishedAtEnd,

        @Parameter(description = "並び順") @Schema(defaultValue = "-published_at")
        BookOrder order) {
    ...
    public BookSearchQuery toBookSearchQuery(@Nullable Integer offset, @Nullable Integer limit) {
        return new BookSearchQuery(
                Isbn.of(isbn),
                title,
                RangeInteger.of(priceFrom, priceTo),
                status,
                RangeTime.of(publishedAtStart, publishedAtEnd),
                order,
                offset,
                limit);
    }
